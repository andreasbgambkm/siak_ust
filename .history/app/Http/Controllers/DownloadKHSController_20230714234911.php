...
class DownloadKHSController extends Controller
{
    public function show($id)
    {
        ...
        foreach ($khs as $item) {
            ...
            $item->nilai_huruf = '';
            if ($item->N_angka <= 45.5) {
                $item->nilai_huruf = 'E';
            } elseif ($item->N_angka <= 55.5) {
                $item->nilai_huruf = 'D';
            } elseif ($item->N_angka <= 65.5) {
                $item->nilai_huruf = 'C';
            } elseif ($item->N_angka <= 70.5) {
                $item->nilai_huruf = 'C+';
            } elseif ($item->N_angka <= 75.5) {
                $item->nilai_huruf = 'B';
            } elseif ($item->N_angka <= 80.5) {
                $item->nilai_huruf = 'B+';
            } elseif ($item->N_angka > 80.5) {
                $item->nilai_huruf = 'A';
            }
        ...
        <tr>
            ...
            <th rowspan="2" style="text-align: center; width: 160px;">Kelas</th>
            <th colspan="2" style="text-align: center; width: 40px;">Kredit(K) </th>
            <th colspan="2" style="text-align: center; width: 100px;">Sem</th>
            <th colspan="2" style="text-align: center; width: 100px;">Nilai Huruf</th>
        </tr>';
        ...
        foreach ($dataTabel as $row) {
            ...
            $html .= '<td style="width: 160px;">&nbsp;' . $row['kelas'] . '</td>';
            $html .= '<td style="width: 160px;">&nbsp;' . $row['sks'] . '</td>';
            $html .= '<td style="width: 160px;"> &nbsp; ' . $row['semester'] . '</td>';
            $html .= '</tr>';

            // Increment counter
            $no++;
        }
        $pdf->Cell(0, 0, 'Jumlah (sks)', 0, 0, 'L');
        $pdf->SetXY(110, 108);
        $pdf->Cell(0, 0, ': '.$totalSks, 0, 0, 'L');
        $pdf->SetXY(10, 120);
        $pdf->Cell(0, 0, 'IP Semester', 0, 0, 'L');
        $pdf->SetXY(50, 80);
        $pdf->Cell(0, 0, ': '.$ip, 0, 0, 'L');
        $pdf->SetXY(115, 80);
        $pdf->Cell(0, 0, 'Maks. sks Semester Berikutnya', 0, 0, 'L');
        $pdf->SetXY(50, 87);
        $pdf->Cell(0, 0, ': '.$nama, 0, 0, 'L');
        $pdf->SetXY(115, 87);

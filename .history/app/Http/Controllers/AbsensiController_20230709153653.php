...
class AbsensiController extends Controller
{
    ...
    ->get();
        ...
        foreach ($response['matakuliah'] as &$matakuliah) {
            $absensi = $matakuliah['absensi'];
            $totalStatus = array_sum($absensi);
            $alphaCount = $absensi['alpha'];
            $presentase = ($totalStatus - $alphaCount) / $totalStatus * 100;
            $matakuliah['presentase'] = number_format($presentase, 2);
        }

...
class RincianAbsenController extends Controller
{
    ...
    public function store(Request $request, $id_jadwal)
    {
        ...
        $tglAbsensiDosen = strtotime($absensiDosen->tgl);
        $jamAbsensiDosen = strtotime($absensiDosen->jam);

        if ($tglAbsensiDosen > $now || ($tglAbsensiDosen == $now && $jamAbsensiDosen > $now)) {
            return response()->json([
                'message' => 'Absensi belum dimulai.',
            ], 400);
        }

        $batasWaktuAbsen = $jamAbsensiDosen + (25 * 60); // Menambahkan 25 menit ke dalam waktu absensi

        if ($now > $batasWaktuAbsen) {
            $status = 'A';
        } else {

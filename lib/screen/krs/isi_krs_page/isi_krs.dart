...
class SiakIsiKRS extends StatefulWidget {
 final int totalSKSTelahDiambil;


  const SiakIsiKRS({super.key, this.totalSKSTelahDiambil = 0});
...
class _SiakIsiKRSState extends State<SiakIsiKRS> {
  ...
  Krs? _krs;
  IsiKRSModel? _isiKrs;
  KrsDiambilData? _krsDiambilData;
  ...
  void _applyFilters() {
    ...
    if (selectedSks > 0) {
      filteredList = filteredList.where((jadwalKrs) => jadwalKrs.sks == selectedSks).toList();
    }
  ...
  void toggleReset() {
    setState(() {
      ...
      if (resetChecked) {
        ...
        totalSKSIsiKRS = 0;
        totalSKSIsiKRS = totalSKSIsiKRS + widget.totalSKSTelahDiambil;
  ...
  void initState() {
    ...
    print(widget.totalSKSTelahDiambil);
    totalSKSIsiKRS = totalSKSIsiKRS + widget.totalSKSTelahDiambil;
  ...
  Future _getAllData() async {
    await _getIsiKrsDataFromServer();
    _loadIsiKrsDataFromLocal();

  }
  Future<IsiKRSModel?> _getIsiKrsDataFromServer() async {
    ...
    if (token != null && token.isNotEmpty) {
      ...
      final storedIsiKrs = prefs.getString('isiKrs');
      if (storedIsiKrs != null && storedIsiKrs.isNotEmpty) {

        return IsiKRSModel.fromJson(jsonDecode(storedIsiKrs));
      } else {
        final data = await _databaseHelper.fetchIsiKrsMahasiswa(token);
        ...
        if (data != null && data.isNotEmpty) {
          ...
          await prefs.setString('isiKrs', jsonData);
                  ...
                  Padding(
                    ...
                    child: ListView.builder(
                      ...
                      itemBuilder: (context, index) {
                        ...
                        bool sudahDiambil = jadwalList.sudahDiambil == 1;
                          ...
                          child: Padding(
                            ...
                            child: Row(
                              children: [
                                ...
                                if (sudahDiambil) // Tampilkan teks "Sudah Diambil" jika sudahDiambil bernilai true
                                  const Padding(
                                    padding: EdgeInsets.only(left: 8),
                                    child:  Icon(Icons.check, color: SiakColors.SiakGreenDark,),
                                  ),
                                if (!sudahDiambil) // Tampilkan Checkbox jika sudahDiambil bernilai false
                                  Checkbox(
                                    ...
                                    onChanged: (value) {
                                      setState(() {
                                        if (value ?? false) {
                                          ...
                                          print(selectedJadwalIds);
                                          print(totalSKSIsiKRS);
                                          if (value == true && totalSKSIsiKRS > 24) {
                                        ...
                                        if (totalSKSIsiKRS > 24) {
                                          // Jika total SKS melebihi 24, ubah value kembali menjadi false (unchecked)
                                          value = false;
                                          selectedJadwalIds.remove(jadwalList.id);
                                          showDialog(
                                            context: context,
                        ...
                        if (selectedJadwalIds.isEmpty && totalSKSIsiKRS == 0) {
                          ...
                          if (ipMahasiswa != null && maxSks != null) {
                            if (ipMahasiswa < 3.0 && maxSks > 20) {
                              ...
                              await  _databaseHelper.postIsiKrs(selectedJadwalIds, token);

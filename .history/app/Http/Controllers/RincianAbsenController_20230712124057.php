...
class RincianAbsenController extends Controller
{
    ...
    public function store(Request $request, $id_jadwal)
    {
        ...
        $jakartaTimeZone = new CarbonTimeZone('Asia/Jakarta');
        Carbon::setTimezone($jakartaTimeZone);
        ...
        $jamAbsensiDosen = Carbon::parse($tanggal . ' ' . $jam);
        $jamToleransi = $jamAbsensiDosen->addMinutes(25);


        if ($now->gt($jamToleransi)) {

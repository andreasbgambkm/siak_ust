...
class RincianAbsenController extends Controller
{
    ...
    public function store(Request $request, $id_jadwal)
    {
        ...
        // Lakukan query untuk mendapatkan data absensi berdasarkan npm dan id_jadwal
        $absensiMhs = AbsensiMahasiswa::where('npm', $npm)->where('id_jadwal', $id_jadwal)->first();

        // Periksa apakah data absensi ditemukan
        if (!$absensiMhs) {
            // Jika data absensi tidak ditemukan, maka buat data baru untuk npm dan id_jadwal yang dimaksud
            $absensiMhs = new AbsensiMahasiswa();
            $absensiMhs->npm = $npm;
            $absensiMhs->id_jadwal = $id_jadwal;
            $absensiMhs->status = $status;
            $absensiMhs->save();
        ...
        // Jika data absensi ditemukan, lanjutkan dengan perubahan status sesuai dengan npm dan id_jadwal
        $statusSebelum = $absensiMhs->status;

        if ($status === 'H' || $status === 'I' || $status === 'A' || $status === 'S') {
            // Tambahkan status baru ke status sebelumnya jika memenuhi kondisi yang diinginkan
        ...
        // Jika status yang dimasukkan tidak valid
        return response()->json([

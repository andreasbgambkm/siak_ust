...
class RincianAbsenController extends Controller
{
    ...
    public function store(Request $request, $id_jadwal)
    {
        ...
        // Menggabungkan tanggal dan jam menjadi satu string dalam format "Y-m-d H:i:s"
        $dateTimeString = $tanggal . ' ' . $jam;

        // Mengubah string datetime menjadi timestamp
        $dateTimeTimestamp = strtotime($dateTimeString);

        // Menambahkan 25 menit (25 * 60 detik) pada timestamp datetime
        $extendedTimestamp = $dateTimeTimestamp + (25 * 60);

        // Mengubah timestamp yang telah ditambahkan 25 menit menjadi string datetime baru
        $extendedDateTimeString = date('Y-m-d H:i:s', $extendedTimestamp);

        $now = time();

        if ($now > $extendedTimestamp) {
            // Melakukan pengecekan jika sudah melewati tanggal dan waktu ditambah 25 menit
            // Update status pada tabel absensimahasiswa menjadi "A"
        ...
        if (!$status || !in_array($status, ['H', 'I', 'A', 'S'])) {
            return response()->json([

...
class RincianAbsenController extends Controller
{
    ...
    public function store(Request $request, $id_jadwal)
    {
        ...
        $datetimeAbsensiDosen = strtotime($absensiDosen->tgl . ' ' . $absensiDosen->jam);

        if ($datetimeAbsensiDosen > $now) {
            return response()->json([
                'message' => 'Absensi belum dimulai.',
            ], 400);
        }

        $batasWaktuAbsen = $datetimeAbsensiDosen + (25 * 60); // Menambahkan 25 menit ke dalam waktu absensi

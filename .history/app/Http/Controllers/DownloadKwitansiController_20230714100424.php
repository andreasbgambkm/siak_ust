...
class DownloadKwitansiController extends Controller
{
    public function uangkuliahpokok($id)
    {
        ...
        $nama = $profile->nm_mhs;
        $foto = $profile->Foto;
        $idDosen = $profile->dosen_pembimbing;

        $getNamaDosen = Dosen::findOrFail($idDosen);
        $dosenPembimbing =  $getNamaDosen->nmdos;
        ...
        $pdf->SetCreator($nama);
        $pdf->SetAuthor($uid);
        $pdf->SetTitle('Kuitansi Sementara UKP');
        $pdf->SetSubject('Kuitansi Sementara UKP');
